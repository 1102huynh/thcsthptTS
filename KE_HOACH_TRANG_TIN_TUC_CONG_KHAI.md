...
**Phiên bản 1.3 — ngày 05/09/2026**
*(v1.3: **P1 + P2 + P3 xong**; **P4 gần xong** — Spring Cache, `viewCount` gộp/bất đồng bộ, `sitemap.xml` động (backend + build-time), **prerender route công khai** (bot Facebook/Zalo đọc được OG) và **WYSIWYG editor** (TipTap) đều đã làm. Chỉ còn QA responsive/tốc độ thật trên domain thật + điền thông tin thật của trường (tên/địa chỉ/SĐT/email — cần chốt với nhà trường). Xem "TRẠNG THÁI TRIỂN KHAI" cuối tài liệu.)*
...
### ⏳ P4 — SEO nâng cao & hoàn thiện (MỘT PHẦN — 05/09/2026)
- ✅ **Spring Cache tầng ứng dụng**: `@EnableCaching` (`SchoolManagementApplication`, `ConcurrentMapCacheManager` mặc định — 1 instance, chưa cần Redis). `@Cacheable` trên `NewsService`/`SchoolEventService`/`PublicPortalService` (danh sách công khai, chuyên mục, featured/latest/upcoming, trang chủ, sitemap); mọi thao tác ghi CMS (create/update/publish/unpublish/delete, kể cả chuyên mục) `@CacheEvict(allEntries=true)` đúng cache liên quan. Chi tiết 1 bài/sự kiện (theo slug) **không cache** (đơn giản hoá).
- ✅ **`viewCount` gộp/bất đồng bộ**: `NewsViewCountAggregator` — request công khai chỉ tăng `LongAdder` trong bộ nhớ (`recordView`, không đụng DB); `@Scheduled` (`app.news.view-count-flush-ms`, mặc định 30s) gộp và ghi DB **1 lần UPDATE mỗi bài** (`incrementViewCountBy`, `clearAutomatically=true` để tránh entity cache cũ). Bài đọc nhiều không còn khoá ghi từng lượt xem.
- ✅ **`sitemap.xml` động**: `GET /v1/public/sitemap.xml` (`PublicPortalService.sitemapXml()`, cache 1h) — route tĩnh + 1 URL tuyệt đối cho **mỗi** bài/sự kiện đã đăng (base URL từ `app.frontend.site-url`/`FRONTEND_SITE_URL`). File tĩnh `frontend/public/sitemap.xml` (chỉ route) vẫn giữ làm phương án dự phòng khi chưa cấu hình proxy — xem mục cấu hình bên dưới.
- Test: `PublicPortalIntegrationTest` +2 (viewCount qua flush trực tiếp, sitemap có đủ slug) — **10/10 pass** (chạy thật với MySQL cục bộ).
- ✅ **Prerender route công khai** (`frontend/scripts/prerender.mjs`, chạy sau `vite build` — script `build` giờ là `vite build && node scripts/prerender.mjs`): với mỗi route tĩnh + **mỗi bài tin/sự kiện đã đăng** (phân trang qua toàn bộ `/v1/public/news`+`/v1/public/events`, không chỉ trang đầu), ghi `build/<route>/index.html` — sao y `<head>` (script/css đã hash) từ `build/index.html` thật của Vite, chỉ thay `<title>`/`<meta description>`/OG/Twitter/`canonical`, và bơm một bản HTML tĩnh (tiêu đề, ảnh bìa, tóm tắt, nội dung đã sanitize) vào trong `#root` để bot **không chạy JS** (Facebook, Zalo) thấy nội dung thật; trình duyệt thật vẫn nhận đúng bundle SPA và React tự mount đè lên như bình thường. Cũng ghi lại `build/sitemap.xml` (URL tuyệt đối, kể cả từng bài/sự kiện) — **cùng host với trang**, không cần proxy như bản backend.
  - **Giới hạn đã ghi rõ trong script**: chỉ chụp nội dung tại **thời điểm build** — bài đăng mới chỉ có preview unfurl sau lần `npm run build` kế tiếp (vẫn xem được bình thường qua SPA/Googlebot ngay lập tức, chỉ preview Facebook/Zalo bị trễ tới khi build lại).
  - **Yêu cầu hạ tầng khi lên thật**: web server phục vụ `build/` phải theo thứ tự `try_files`-style: file khớp chính xác → `<path>/index.html` → `index.html` gốc (SPA fallback) — **đúng mặc định của Nginx/Apache/Netlify/Vercel**, nhưng **`vite preview` KHÔNG làm vậy** (luôn fallback thẳng về `index.html` gốc, bỏ qua snapshot). Đã kiểm chứng bằng `frontend/scripts/serve-build.mjs` (server tĩnh ~80 dòng, không thêm dependency, dùng đúng thứ tự try_files) — chạy `npm run preview:prerendered` để xem thật ở local; `npm run preview` (vite) vẫn dùng được để xem SPA nhưng sẽ không thấy các trang đã prerender.
  - Test thủ công: dựng backend thật + `npm run build` + `npm run preview:prerendered`, xác nhận `/tin-tuc/<slug>` trả đúng `<title>`/OG/nội dung, `/login` (route không prerender) vẫn fallback SPA đúng, asset tĩnh (`favicon.ico`) vẫn phục vụ trực tiếp.
- ✅ **WYSIWYG editor**: `components/shared/RichTextEditor.jsx` (TipTap v3: `@tiptap/react` + `@tiptap/starter-kit` + `@tiptap/extension-image` + `Placeholder` từ `@tiptap/extensions`, đều đã có sẵn qua starter-kit ngoại trừ `extension-image`) thay textarea+"Xem trước" trong `NewsManagement.jsx`/`EventManagement.jsx`. Toolbar **chỉ có đúng những gì `HtmlSanitizerService` giữ lại** (đậm/nghiêng/gạch chân, H2/H3, danh sách, trích dẫn, đường kẻ ngang, liên kết, ảnh, hoàn tác/làm lại) — cố ý bỏ khối code/màu chữ/font vì backend sẽ âm thầm lọc bỏ. Chèn ảnh dùng lại `mediaCmsService.upload` sẵn có; style vùng soạn thảo dùng chung `richTextProse.js` với `RichHtml.jsx` (soạn thảo trông giống hệt lúc hiển thị công khai). Bundle TipTap (~130KB gzip) tự tách chunk riêng, chỉ tải khi mở trang quản trị Tin tức/Sự kiện (đã xác nhận qua `npm run build`).
  - Test: `RichTextEditor.test.jsx` (3 test — render nội dung ban đầu + toolbar, nút chèn ảnh chỉ xuất hiện khi có `onUploadImage` và gọi đúng callback, bấm nút định dạng không lỗi). `npm test`: **39/39 pass**.
- ⏳ **Còn lại, cần dữ liệu/quyết định thật, chưa làm**: QA responsive thật + đo tốc độ + kiểm tra unfurl Facebook/Zalo thật (cần domain thật lên mạng); điền thông tin thật (địa chỉ/SĐT/email trường trong `PublicLayout`+`ContactPage`, tên trường trong `Seo.jsx` `SITE_NAME` — hiện vẫn là "Trường THCS & THPT" placeholder) — **cần chốt với nhà trường**.

### Việc cấu hình khi lên thật
- `APP_CORS_ALLOWED_ORIGINS` = domain cổng công khai (cho phép trình duyệt khách gọi API).
- `FRONTEND_SITE_URL` = domain cổng công khai **không dấu `/` cuối** (VD `https://truongxyz.edu.vn`) — dùng để dựng URL tuyệt đối trong `sitemap.xml` động.
- Ảnh OG cần URL tuyệt đối — `publicService.mediaUrl()` đã ghép `VITE_API_BASE_URL`; đảm bảo biến này trỏ domain thật.
- **Sitemap**: `frontend/scripts/prerender.mjs` (chạy trong `npm run build`) giờ tự ghi `build/sitemap.xml` đầy đủ (route tĩnh + từng bài/sự kiện, URL tuyệt đối theo `PRERENDER_SITE_URL`/`FRONTEND_SITE_URL`) — **không cần proxy**, cùng host mặc định. `GET /v1/public/sitemap.xml` (backend) vẫn còn, luôn mới nhất (không phụ thuộc lần build gần nhất) nhưng nằm dưới path backend (`/api/...`) nên **muốn dùng nó thay cho bản build-time thì phải reverse-proxy** `https://<domain>/sitemap.xml` → `http://backend/api/v1/public/sitemap.xml` (chuẩn sitemaps.org: sitemap chỉ liệt kê được URL trong phạm vi thư mục nó nằm).
- **QUAN TRỌNG — web server phải phục vụ `build/` theo thứ tự `try_files`**: file khớp chính xác → `<path>/index.html` → `index.html` gốc. Đây là mặc định của Nginx (`try_files $uri $uri/index.html /index.html;`)/Apache/Netlify/Vercel/IIS-URL-Rewrite, nhưng **KHÔNG phải mặc định của `vite preview`** (luôn fallback thẳng `index.html`, bỏ qua snapshot prerender) — dùng `npm run preview:prerendered` (script tĩnh tự viết, `frontend/scripts/serve-build.mjs`) để xem đúng ở local, còn lên thật thì cấu hình `try_files` như trên ở web server thật.
- `PRERENDER_API_BASE_URL` (mặc định theo `VITE_API_BASE_URL`) + `PRERENDER_SITE_URL` (mặc định theo `FRONTEND_SITE_URL`) — env cho `npm run build` biết gọi API nào và domain thật nào khi build production (build lúc CI/CD cần backend đã chạy được, nếu không script tự bỏ qua phần prerender bài/sự kiện + sitemap, chỉ giữ route tĩnh — không làm hỏng build).
