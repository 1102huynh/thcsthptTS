...
**Phiên bản 4.24 — ngày 06/09/2026**
*(v4.24: **vá lỗi thật tìm được khi tự test v4.23 trên trình duyệt** — mọi `AccessDeniedException` trong toàn app (không riêng H.3.1) trả về nguyên văn tiếng Anh "You do not have permission to perform this action" thay vì tiếng Việt, do `GlobalExceptionHandler` hard-code chuỗi này bất kể exception message thật là gì. Đã sửa sang "Bạn không có quyền thực hiện thao tác này." `mvn test` **274/274**. v4.23: **rà soát H.3.1 theo Thông tư 32/2020/TT-BGDĐT + thực tế "sổ đầu bài"** — mở rộng quyền Điểm danh cho GVBM có `TeachingAssignment` (không chỉ GVCN), khớp đúng cách trường Việt Nam ghi điểm danh theo tiết. `mvn test` **274/274** (2 lỗi cũ không đổi); `npm test` **76/76**. v4.22: **vá lỗi thật tìm được khi kiểm tra trực tiếp trên trình duyệt** — GVBM (teacher2) không tải được danh sách lớp để nhập điểm dù đã được phân công dạy, do endpoint `GET /v1/students/class/{className}/section/{section}` (dùng chung bởi Điểm danh và Bảng điểm) bị khoá theo GVCN ở v4.20, xung đột với yêu cầu GVBM ở v4.21. Đã gỡ khoá GVCN khỏi endpoint dùng chung này — bảo vệ thực tế vẫn nằm ở từng endpoint ghi (điểm danh/điểm số) như thiết kế ban đầu. `mvn test` **265/265** (2 lỗi cũ không đổi). v4.21: **H.3.1 hoàn tất — GVBM chỉ nhập điểm môn mình dạy**, dựa trên `TeachingAssignment` (`TeacherAssignmentGuard` mới). `npm test` **75/75**. v4.20: thu hẹp quyền TEACHER theo lớp chủ nhiệm (GVCN) — Quản lý học sinh/Điểm danh/Xét lên lớp chỉ còn phạm vi lớp mình chủ nhiệm; Thời khoá biểu chỉ xem đúng lịch dạy của chính mình. v4.19: thêm React Error Boundary tổng. v4.18: kiểm tra thủ công toàn bộ chức năng trên trình duyệt thật (cả 7 vai trò), vá 4 lỗi thật. v4.17: rate-limit `/v1/auth/login`. v4.16: review phát hiện B0 vẫn chặn. v4.15: D6 xong. v4.14: Spring Boot 3.5.3 + react-router-dom 7.18.3. Còn chặn duy nhất: B0. Xem "Nhật ký thay đổi".)*
...
- **D6.** ✅ **XONG (v4.15)** — trang `UserAccountManagement` (`/accounts`, ADMIN-only): xem mọi tài khoản mọi vai trò (lọc theo vai trò + tìm theo tên đăng nhập/họ tên/email, phân trang server-side) + khoá/mở đăng nhập. Backend: `GET /v1/users/search` (Page, tách khỏi `GET /v1/users?role=` mà `ParentManagement.jsx` đang dùng — không đổi endpoint cũ), `PUT /v1/users/{id}/enabled`. `enabled=false` chặn đăng nhập ngay (cơ chế `UserDetails.isEnabled()` đã có sẵn, không cần sửa filter/security config) — chỉ chặn lần đăng nhập tiếp theo, không thu hồi JWT đã phát hành (ghi cùng F3). ADMIN không thể tự khoá chính mình (chặn cả backend lẫn ẩn nút ở FE). Test: `AuthenticationServiceTest` (+3), `UserControllerIntegrationTest` (+4, gồm ca khoá xong → login 401), `UserAccountManagement.test.jsx` (5 ca).

**DoD:** ✅ **ĐẠT** — nộp hồ sơ online → duyệt → tạo tài khoản HS; tải học bạ PDF & điểm danh Excel; đặt lại mật khẩu qua email; ADMIN xem/khoá/mở mọi tài khoản.
...
4. ~~[Rà service] Endpoint chỉ `authenticated()`: `PUT /v1/notifications/{recipientId}/read` và `POST /v1/documents` → cần service kiểm tra chủ sở hữu.~~ ✅ **[v4.16 — xác minh lại, hoá ra đã làm từ trước, mục này lỗi thời]** `NotificationService.markAsRead()` đã ném `AccessDeniedException` nếu `row.getRecipient().getId()` khác người gọi (dòng 132-134); `DocumentService.upload()` đã gọi `enforceOwnerAccess(ownerType, ownerId, uploader, true)` trước khi lưu (dòng 89). Không phải lỗ hổng — chỉ là dòng ghi chú trong tài liệu chưa được cập nhật khi code đã vá.
...
7. ~~[Frontend] Chưa chặn route theo vai trò (A3) — backend chặn nhưng UI vẫn cho bấm rồi mới 403.~~ ✅ **Lỗi thời, trùng A3** — `ProtectedRoute.jsx` đã làm từ v4.5 (xem A.2/A.3 mục A3), `guarded()` trong `App.jsx` bọc mọi route kể cả route mới `/accounts` (D6, v4.15).
8. ~~`POST /v1/auth/login` không có rate-limit.~~ ✅ **[v4.17] Đã vá — `LoginRateLimitFilter`** (theo đúng pattern `ForgotPasswordRateLimitFilter`, khoá theo IP), đăng ký trước `JwtAuthenticationFilter` trong `SecurityConfig`. Mặc định **20 lần/5 phút mỗi IP** — rộng rãi hơn nhiều so với `forgot-password`/`admission` (3-5 lần/60 phút) vì đăng nhập là hành động người dùng hợp lệ làm liên tục, thường từ IP NAT chung của cả trường; đủ chặn thử hàng trăm-nghìn mật khẩu theo kịch bản trong khi vẫn rộng rãi cho đăng nhập đồng thời thật. `POST /v1/auth/register` vẫn chưa giới hạn (rủi ro thấp hơn — spam tài khoản STUDENT, không phải brute-force — chưa xử lý trong lượt này). Test `LoginRateLimitIntegrationTest` (429 ở lần thử thứ 3 với ngưỡng test = 2); `application-test.yml` thêm override `max-requests: 1000` (cùng lý do với admissions/forgot-password — bộ đếm dùng chung Spring context).
...
- **3.1. Thu hẹp quyền giáo viên theo phân công** — ✅ **XONG (v4.20 → v4.23)**, theo yêu cầu trực tiếp của người dùng (chặn thẳng, không "cảnh báo trước" như dự tính ban đầu):
  - **[v4.20]** GVCN chỉ quản lý học sinh/điểm danh/xét lên lớp cho lớp mình chủ nhiệm (`TeacherHomeroomGuard` mới, dùng chung — trước đó chỉ Hạnh kiểm có logic này, nay refactor để `ConductRecordService` cũng dùng guard này); Thời khoá biểu chỉ xem đúng lịch dạy của chính mình, không browse theo lớp bất kỳ nữa.
  - **[v4.21]** GVBM chỉ nhập điểm cho (lớp, môn, học kỳ) mà mình có `TeachingAssignment` (`TeacherAssignmentGuard` mới) — chỉ giới hạn ghi (tạo/sửa/xoá `GradeRecord`), không giới hạn đọc, cùng nguyên tắc đã áp dụng cho GVCN.
  - **[v4.22]** Vá xung đột: endpoint đọc roster dùng chung giữa Điểm danh và Bảng điểm từng bị khoá theo GVCN, khiến GVBM không tải được danh sách để nhập điểm — đã gỡ, bảo vệ chuyển hẳn về từng endpoint ghi.
  - **[v4.23]** Rà soát lại theo Thông tư 32/2020/TT-BGDĐT + thực tế ghi "sổ đầu bài": **mở rộng Điểm danh cho GVBM** có `TeachingAssignment` cho lớp đó (không chỉ GVCN) — khớp đúng cách trường Việt Nam điểm danh theo từng tiết (giáo viên dạy tiết nào ghi vắng tiết đó), GVCN chỉ là 1 trong 2 điều kiện đủ, không còn là điều kiện duy nhất. Hạnh kiểm, Xét lên lớp, Điểm số, Thời khoá biểu đã rà soát và khớp đúng quy định/thực tế, không đổi.
...
| Điểm (TT22) — nhập / xem | ✔ / ✔ | – / **✔** | ✔ / ✔ *(chỉ lớp/môn/kỳ có TeachingAssignment — v4.21)* | – / *mình* | – / *con* | – | – |
| Điểm danh — nhập / xem | ✔ / ✔ | – / **✔** | ✔ / ✔ *(lớp mình chủ nhiệm HOẶC có phân công giảng dạy — v4.20, mở rộng v4.23)* | – / *mình* | – / *con* | – | – |
...
| Quản lý tài khoản | `POST /v1/users`, `GET /v1/users/search`, `PUT /v1/users/{id}/enabled` *(D6)* | ✅ | | | | | | |
...
- **v4.24 (06/09/2026):** Người dùng yêu cầu "tự test và điều chỉnh luôn nếu sai" sau v4.23. Test qua trình duyệt thật (Claude-in-Chrome): đăng nhập `teacher2` (GVBM), điểm danh ngày ngoài học kỳ được phân công → bị chặn đúng thiết kế (403), nhưng thông báo lỗi hiện ra là **"You do not have permission to perform this action"** — tiếng Anh, không nhất quán với phần còn lại của app (đã có tiền lệ vá tương tự cho lỗi đăng nhập ở A6/v4.18).
  - **Nguyên nhân:** `GlobalExceptionHandler.handleAccessDeniedException` hard-code chuỗi tiếng Anh này cho **mọi** `AccessDeniedException` trong toàn hệ thống (không riêng gì Điểm danh) — bỏ qua hoàn toàn message thật của exception. Toàn bộ message chi tiết đã viết cẩn thận trong `TeacherHomeroomGuard`/`TeacherAssignmentGuard`/`StudentAccessGuard`/`ConductRecordService` từ đầu tới giờ (v4.20-v4.23) chưa từng đến được người dùng.
  - **Cách vá:** đổi chuỗi hard-code sang tiếng Việt "Bạn không có quyền thực hiện thao tác này." — không dùng `ex.getMessage()` trực tiếp vì các message đó viết bằng tiếng Anh cho lập trình viên đọc log, không phải để hiển thị; dịch từng cái ra tiếng Việt tốn công hơn nhiều so với lợi ích, nên chọn 1 thông báo chung tiếng Việt, cùng triết lý với `handleBadCredentialsException` đã làm trước đó.
  - `mvn test` **274/274** (2 lỗi cũ không đổi, không có test nào phụ thuộc chuỗi cũ).
- **v4.23 (06/09/2026):** Người dùng yêu cầu "rà soát lại và vẫn theo nguyên tắc trước giờ là: phù hợp với giáo dục ở Việt Nam" sau khi H.3.1 hoàn tất (v4.20-v4.22). Tra cứu Thông tư 32/2020/TT-BGDĐT (Điều lệ trường THCS/THPT) + cách ghi "sổ đầu bài" thực tế: Hạnh kiểm (GVCN "nhận xét, đánh giá xếp loại"), Xét lên lớp (GVCN "đề nghị danh sách học sinh được lên lớp"), Điểm số (GVBM theo phân công), Thời khoá biểu (giáo viên chỉ xem, BGH quyết định) — **đều đã khớp đúng**. Riêng **Điểm danh** đang giới hạn "chỉ GVCN" (đúng yêu cầu ban đầu của người dùng) **không khớp thực tế**: sổ đầu bài ghi điểm danh theo từng tiết, do giáo viên bộ môn đang dạy tiết đó ghi nhận vắng mặt, không chỉ riêng GVCN. Đã hỏi lại người dùng (AskUserQuestion) và được xác nhận: **mở rộng Điểm danh cho GVBM có `TeachingAssignment`**.
  - **Ràng buộc kỹ thuật:** `Attendance` không có trường môn học/tiết (chỉ theo ngày) — khác sổ đầu bài thật (theo tiết). Điều kiện GVBM vì vậy kiểm theo **lớp + học kỳ chứa ngày điểm danh** (bất kể môn), không khớp tuyệt đối "đúng tiết" nhưng khớp đúng nguyên tắc cốt lõi.
  - **Backend:** `AttendanceService` gộp 4 chỗ kiểm quyền (mark/markForClass/update/delete) thành 1 method `enforceCanTouchAttendance` — TEACHER được điểm danh nếu là GVCN (như trước) **HOẶC** có `TeachingAssignment` cho lớp đó trong học kỳ chứa ngày điểm danh. Thêm `TeacherHomeroomGuard.isHomeroomClassNameSection` (biến thể không throw của `enforceHomeroomClassNameSection`, để thử tiếp điều kiện GVBM trước khi từ chối hẳn) và `TeacherAssignmentGuard.hasAssignmentForClass` (biến thể không phân biệt môn học của `enforceHasAssignment`, dùng cho trường hợp điểm danh không có khái niệm môn). `SemesterRepository` thêm method suy ra học kỳ theo ngày — trả về `List`, không phải `Optional`, vì không gì đảm bảo các học kỳ không chồng lấn ngày tháng (gặp thật khi test: DB dev có học kỳ chồng lấn, ném `NonUniqueResultException` nếu dùng `Optional`).
  - **Frontend:** `AttendanceManagement.jsx` — Select Lớp giờ là **hợp** của lớp mình chủ nhiệm và lớp có `TeachingAssignment` (không lọc theo học kỳ ở FE — chỉ backend kiểm đúng học kỳ theo ngày cụ thể lúc lưu).
  - Test mới: `TeacherHomeroomGuardTest`/`TeacherAssignmentGuardTest` (ca cho 2 method mới), `AttendanceIntegrationTest` (bổ sung `AcademicYear`/`Semester`/`Subject`/`TeachingAssignment` vào fixture, 2 ca GVBM mới), `AttendanceManagement.test.jsx` (1 ca GVBM mới).
  - `mvn test` **274/274** (baseline 265 + 9, 2 lỗi cũ không đổi); `npm test` **76/76** (baseline 75 + 1); `npm run build` sạch.
  - Nguồn tham khảo: [Thông tư 32/2020/TT-BGDĐT (LuatVietnam)](https://luatvietnam.vn/giao-duc/thong-tu-32-2020-tt-bgddt-dieu-le-truong-thcs-va-thpt-190977-d1.html), [nhiệm vụ GVCN (LawNet)](https://lawnet.vn/giao-duc/giao-vien-chu-nhiem-o-truong-thcs-co-nhung-nhiem-vu-gi-359.html), [cách ghi sổ đầu bài (MISA EMIS)](https://emishelp.misa.vn/kb/928/).
- **v4.22 (06/09/2026):** **Lỗi thật tìm được khi kiểm tra trực tiếp trên trình duyệt** sau khi merge v4.20+v4.21 — người dùng yêu cầu "test lại giúp tôi", phát hiện đăng nhập `teacher2` (GVBM có phân công dạy Ngữ văn lớp 10-A, không phải GVCN) vào trang Bảng điểm: Select Lớp/Môn học hiện đúng "10-A / Ngữ văn" (lọc theo `TeachingAssignment` đúng như v4.21), nhưng bảng nhập điểm báo **"Lớp này chưa có học sinh nào"** dù lớp có 25 học sinh.
  - **Nguyên nhân:** `GradeManagement.jsx` và `AttendanceManagement.jsx` dùng chung 1 hàm frontend (`studentService.getByClass`) để tải danh sách học sinh của lớp, ánh xạ tới cùng 1 endpoint backend `GET /v1/students/class/{className}/section/{section}`. Ở v4.20, endpoint này bị khoá theo GVCN (`TeacherHomeroomGuard.enforceHomeroomClassNameSection`) để phục vụ đúng yêu cầu của Điểm danh ("chỉ điểm danh học sinh lớp mình chủ nhiệm") — nhưng Bảng điểm cần ngược lại: bất kỳ GVBM nào có `TeachingAssignment` cho lớp đó (không cần là GVCN) đều phải xem được danh sách để nhập điểm. Endpoint dùng chung không thể đồng thời thoả cả 2 quy tắc trái ngược nhau cho 2 tính năng khác nhau.
  - **Cách vá:** gỡ hẳn khoá GVCN khỏi `GET /v1/students/class/{className}/section/{section}` (trả về không giới hạn cho TEACHER như trước v4.20) — vì bảo vệ thực sự chưa từng nằm ở bước đọc danh sách, mà nằm riêng ở từng endpoint **ghi**: `AttendanceService.markAttendanceForClass` đã tự kiểm GVCN độc lập, `GradeRecordService` đã tự kiểm `TeachingAssignment` độc lập (`TeacherAssignmentGuard`, v4.21) — cả hai không phụ thuộc vào endpoint đọc danh sách này. Không trang nào khác dùng endpoint này (`StudentManagement.jsx` — "Quản lý học sinh" — dùng `GET /v1/students`, vẫn lọc theo GVCN đúng như thiết kế, không đổi). `GET /v1/classes/{id}/students` (`ClassManagement.jsx`) vẫn khoá GVCN riêng, không bị ảnh hưởng.
  - **Vì sao test tự động không bắt được:** test tích hợp của Bảng điểm (`GradeRecordIntegrationTest`) tạo điểm trực tiếp bằng id học sinh đã biết, bỏ qua bước tải danh sách lớp; test frontend (`GradeManagement.test.jsx`/`AttendanceManagement.test.jsx`) mock thẳng `studentService.getByClass` nên không chạm tới quyền thật ở backend. Chỉ lộ ra khi test đầu-cuối thật trên trình duyệt với dữ liệu seed thật.
  - Cập nhật test: `StudentAccessSecurityTest` (gộp 2 ca cũ theo-GVCN/không-theo-GVCN thành 1 ca xác nhận truy cập không giới hạn).
  - `mvn test` **265/265** (2 lỗi cũ không đổi). Đã test tay lại đầy đủ qua API thật + trình duyệt thật (Claude-in-Chrome): teacher1 (GVCN) — Quản lý học sinh/Điểm danh/Thời khoá biểu/Xét lên lớp đều đúng phạm vi lớp chủ nhiệm; teacher2 (GVBM) — Bảng điểm giờ tải đúng 25 học sinh, nhập và lưu điểm thành công; ADMIN — không bị giới hạn ở bất kỳ đâu.
- **v4.21 (06/09/2026):** **H.3.1 hoàn tất — GVBM (giáo viên bộ môn) chỉ nhập điểm môn mình dạy**, phần còn lại của H.3.1 sau khi xong GVCN (v4.20), theo lựa chọn trực tiếp của người dùng khi được hỏi "làm việc gì tiếp theo".
  - **Backend:** `TeacherAssignmentGuard` mới (`security/`) — cùng style `TeacherHomeroomGuard` nhưng tách riêng (homeroom và teaching-assignment là 2 quy tắc thu hẹp khác nhau trên dữ liệu khác nhau, không phải biến thể của nhau). `GradeRecord` không có FK trực tiếp tới `SchoolClass` và `Student.currentClass` (FK thật) **chưa từng được set ở bất kỳ đâu trong code** — nên guard suy ra lớp từ `Student.className`/`section` (chuỗi) + năm học của `Semester` (`SchoolClassRepository.findByClassNameAndSectionAndAcademicYear`) để tránh nhập nhằng trùng tên lớp qua các năm, rồi kiểm `TeachingAssignmentRepository.existsByTeacherAndSchoolClassAndSubjectAndSemester` (method mới).
  - `GradeRecordService.createGradeRecord` đổi chữ ký để nhận `User requester` (trước đây **hoàn toàn không nhận** — TEACHER tạo điểm cho bất kỳ ai cũng được, không hề bị giới hạn); `updateGradeRecord` kiểm cả tổ hợp (lớp, môn, kỳ) hiện tại lẫn tổ hợp mới (chặn "hijack" sang tổ hợp không được phân công, cùng cách `ConductRecordService.updateConductRecord` đã làm cho GVCN); `deleteGradeRecord` kiểm trước khi xoá. Chỉ giới hạn **ghi** — 4 endpoint đọc điểm giữ nguyên.
  - `GradeRecordController.createGradeRecord` trước đây **không nhận `Authentication`** — đây là lỗ hổng plumbing khiến TEACHER-scoping trước giờ không thể thêm được, đã vá cùng đợt.
  - **Frontend:** `GradeManagement.jsx` — Select Lớp lọc theo lớp có `TeachingAssignment`; Select Môn học lọc chặt hơn theo đúng tổ hợp (lớp, học kỳ) đang chọn; thông báo rỗng khi chưa được phân công. Tiện thể sửa `myStaffId` đang đọc thẳng `localStorage` thay vì `getCurrentUser()` (quy ước cũ trước khi có `useMyHomeroomClasses`, sang đúng chuẩn hiện tại).
  - **Lỗi thật tìm được khi viết test (giống hệt lỗi ở v4.20):** `GradeManagement.jsx` cũng có 2 chỗ `const x = query.data ?? (mảng/Map rỗng)` không memoize + `useEffect` phụ thuộc gọi `setState` vô điều kiện — gây vòng lặp render vô hạn khi TEACHER chưa có phân công (không có lớp nào chọn được). Vá bằng `useMemo`, giống `AttendanceManagement.jsx`/`ConductManagement.jsx` ở v4.20.
  - Test mới: `TeacherAssignmentGuardTest` (unit), mở rộng `GradeRecordIntegrationTest` (3 test `createGradeRecord_*` cũ chuyển từ `@WithMockUser` sang `asUser` — cùng lý do đã gặp ở v4.20 — cộng 5 ca mới); `GradeManagement.test.jsx` (mới, 4 ca).
  - `mvn test` **266/266** (baseline 255 + 11, 2 lỗi cũ không đổi); `npm test` **75/75** (baseline 71 + 4); `npm run build` sạch.
...
- **v4.17 (05/09/2026):** vá lỗ hổng phát hiện ở v4.16 — `POST /v1/auth/login` không có rate-limit.
  ...
  - Test mới: `LoginRateLimitIntegrationTest` (429 ở lần thử thứ 3, ngưỡng test override = 2).
  - `mvn test` **230/230** (baseline 229 + 1, 2 lỗi cũ không đổi); `npm test` 50/50 không đổi (thay đổi chỉ ở backend).
  - **Chưa làm trong lượt này:** `POST /v1/auth/register` vẫn chưa có rate-limit (rủi ro thấp hơn — spam tài khoản STUDENT chứ không phải brute-force mật khẩu).
- **v4.16 (05/09/2026):** review bổ sung sau D6 — người dùng hỏi "ngoài các việc cần chốt nhà cung cấp, còn việc gì khác không", rà lại toàn bộ project lần nữa thay vì chỉ liệt kê từ trí nhớ.
  - Quét mọi controller cho endpoint thiếu `@PreAuthorize` cụ thể (chỉ dựa `authenticated()` mặc định) để tìm IDOR còn sót — soát hết, không có gì mới ngoài các case đã biết và đều đã guard đúng ở tầng service.
  - **Phát hiện thật (mới, actionable):** `POST /v1/auth/login` **không có rate-limit** dù mọi endpoint public nhạy cảm khác (`forgot-password`, `admission`, `contact`, và `change-password` từ v4.12) đều đã có `SlidingWindowRateLimiter` riêng — endpoint dễ bị brute-force mật khẩu nhất trong toàn hệ thống lại là endpoint duy nhất không được bảo vệ. Ghi vào G.4 mục 8 + A.3 mục 8.
  - **Phát hiện tài liệu lỗi thời (không phải lỗ hổng thật):** G.4 mục 4 (ownership check cho `PUT /v1/notifications/{id}/read` + `POST /v1/documents`) và mục 7 (frontend route-guard) — cả hai **đã được vá từ các phiên bản trước**, chỉ là dòng ghi chú trong tài liệu chưa cập nhật theo. Đã sửa lại cho khớp code thật.
  - Tiện thể phát hiện thêm: `esbuild`/`vite` có CVE mức trung bình (devDependency, chỉ ảnh hưởng `npm run dev`, không ảnh hưởng production) — chưa từng ghi vào tài liệu dù đã thấy qua `npm audit` từ lúc làm react-router-dom. Ghi vào A.3 mục 9.
  - `grep` TODO/FIXME/console.log toàn bộ backend + frontend: sạch, không có gì mới.
- **v4.15 (05/09/2026):** **D6 — trang quản trị tài khoản.** B0 (xếp loại học lực) vẫn chặn bởi quyết định ngoài phạm vi code (cần bảng ngưỡng chuyên môn giáo dục) nên chuyển sang làm D6, việc actionable tiếp theo trong roadmap.
  - Backend: `UserRepository.search()` (JPQL, lọc role + q tuỳ chọn); `AuthenticationService.searchUsers()` + `setUserEnabled()` (chặn ADMIN tự khoá chính mình bằng so `targetId` với `actor.getId()` lấy từ JWT, không phải tham số client gửi lên); `UserController`: `GET /v1/users/search` (Page, ADMIN), `PUT /v1/users/{id}/enabled` (ADMIN) + `SetUserEnabledRequest` DTO. **Không đổi** `GET /v1/users?role=` hiện có — `ParentManagement.jsx` đang dùng nó như mảng phẳng không phân trang.
  - `enabled=false` chặn đăng nhập **ngay lập tức** thông qua cơ chế có sẵn của Spring Security (`User.isEnabled()` → `DaoAuthenticationProvider` tự chặn) — không cần sửa `SecurityConfig`/filter. Xác nhận bằng test end-to-end: khoá tài khoản rồi gọi `POST /v1/auth/login` cho tài khoản đó → 401.
  - Frontend: trang mới `UserAccountManagement.jsx` (`/accounts`, ADMIN) theo khung `AuditLogManagement.jsx` (phân trang server-side) + Select lọc vai trò; nút Khoá có `AlertDialog` xác nhận (mẫu từ `StaffManagement.jsx`), nút Mở khoá không cần xác nhận; ẩn/disable nút Khoá ở đúng dòng của ADMIN đang đăng nhập (phòng vệ song song với backend).
  - Test: `AuthenticationServiceTest` (+3), `UserControllerIntegrationTest` (+4), `UserAccountManagement.test.jsx` (5 ca, mới). `mvn test` **229/229** (baseline 222 + 7, 2 lỗi cũ không đổi); `npm test` **50/50**; `npm run build` sạch.
