...
**Phiên bản 4.15 — ngày 05/09/2026**
*(v4.15: **D6 xong** — trang `UserAccountManagement` (`/accounts`, ADMIN): xem/lọc mọi tài khoản + khoá/mở đăng nhập (`GET /v1/users/search`, `PUT /v1/users/{id}/enabled`, tách khỏi `GET /v1/users?role=` hiện có). `mvn test` 229/229 (baseline + 7 ca mới), `npm test` 50/50. v4.14: nâng Spring Boot 3.1.5→3.5.3 + `react-router-dom` 6.14→7.18.3 song song (Quyết định 7–8), phát hiện cả dòng Spring Boot 3.x đã hết hỗ trợ OSS — lên 4.x là sáng kiến riêng (Quyết định 9). v4.12: review toàn bộ hệ thống + PR #17. Còn chặn duy nhất: B0. Xem "Nhật ký thay đổi".)*
